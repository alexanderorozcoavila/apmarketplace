...
class ApmarketplacediscountaddModuleFrontController extends ModuleFrontController
{
    public $php_self;
    ...
    {
        $vars = array();
        if (Tools::getIsset('controller') && Tools::getValue('controller') == 'discountadd') {
            ...
            if ($this->context->cookie->__isset('cookie_vendor')) {
                ...
                if (Tools::isSubmit('add_distcount')) {
                    $this->addDiscount();
                    $vars['notification'] = $this->l('You have successfully submitted the discount');
                }
                $this->setTemplate('module:apmarketplace/views/templates/front/discount/discountadd.tpl');
